...
if [ -f "$WALLPAPER" ]; then
    swww img "$WALLPAPER" --transition-fps 60 --transition-type grow --transition-pos top-right
fi
...
if [ "$THEME" == "Sukuna" ]; then
    GTK_VARS="@define-color accent_color #ff003c; @define-color accent_bg_color #ff003c; @define-color window_bg_color #0f0505; @define-color window_fg_color #f5e0dc;"
    gsettings set org.gnome.gedit.preferences.editor scheme 'oblivion'

elif [ "$THEME" == "Toji" ]; then
    GTK_VARS="@define-color accent_color #9d7cd8; @define-color accent_bg_color #9d7cd8; @define-color window_bg_color #0d0d0f; @define-color window_fg_color #c0caf5;"
    gsettings set org.gnome.gedit.preferences.editor scheme 'solarized-dark'

elif [ "$THEME" == "Nord" ]; then
    GTK_VARS="@define-color accent_color #88c0d0; @define-color accent_bg_color #81a1c1; @define-color window_bg_color #2e3440; @define-color window_fg_color #d8dee9;"
    gsettings set org.gnome.gedit.preferences.editor scheme 'classic'

elif [ "$THEME" == "Gruvbox Material" ]; then
    GTK_VARS="@define-color accent_color #e78a4e; @define-color accent_bg_color #a89984; @define-color window_bg_color #282828; @define-color window_fg_color #dfbf8e;"
    gsettings set org.gnome.gedit.preferences.editor scheme 'tango'

elif [ "$THEME" == "Catppuccin Mocha" ]; then
    GTK_VARS="@define-color accent_color #f5e0dc; @define-color accent_bg_color #b4befe; @define-color window_bg_color #1e1e2e; @define-color window_fg_color #cdd6f4;"
    gsettings set org.gnome.gedit.preferences.editor scheme 'oblivion'
fi
...
# --- 6. Reload Services ---
# Force Thunar and Gedit to restart to apply new internal colors
hyprctl reload
